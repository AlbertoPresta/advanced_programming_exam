...
public:
    ...
    void info() const{cout<<"first "<<first->key<<"   root "<<root->key<<"    size "<<size_tree<<endl;}
    ...
    Tree(K& o):
    first{nullptr},root{nullptr},size_tree{0},oper{o} {}
    
    Tree():
    first{nullptr},root{nullptr},size_tree{0},oper{null} {}
    ...
    //metodi di iterator
    iterator begin() const {return iterator{first,*this};}
...
public:
    ...
    const W& operator[] (const T& k) const noexcept {
        Node* p = find(k).node();
...
void Tree<T,W,K>::insert(T k,W v)  {
    ...
    if (size_tree==0) {Node* elem=new Node{k,v}; first=elem ; root=elem; size_tree=1;return;};
    Node* p{root};
