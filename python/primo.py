...
The Postcard format is "date:$(DATE); from:$(SENDER); to:$(RECEIVER);" (e.g., "date:2009-12-24; from:Daisy; to:Donald;"). For the sake of simplicity,
...
Class PostcardList must manage the I/O file through the following member functions. Note that you are free to choose the appropriate arguments,
...
class PostcardList:
    ...
    def parsePostcards(self):
        for i in range(len(self._postcards)):
            ...
            fr="from:"
            vir=";"
            begin = self._postcards[i].find(fr)+5
            end = self._postcards[i].find(vir,begin)
            sender=self._postcards[i][begin:end]
            ...
            fr="to:"
            vir=";"
            begin = self._postcards[i].find(fr)+3
            end = self._postcards[i].find(vir,begin)
            receiver=self._postcards[i][begin:end]
    ...
    def getPostcardsByDateRange(self,date_range):
        ...
        date_in_range=[]
        for i in ord_keys:
            if date_range[0]<=i and i<=date_range[1]:
                for j in self._date[i]:
                    date_in_range.append(self._postcards[j])
    ...
    def getPostcardsBySender(self, sender):
        ...
        sender_list=sorted(sender_list)
        return sender_list
    ...
    def getPostcardsByReceiver(self, receiver):
        ...
        receiver_list=sorted(receiver_list)
